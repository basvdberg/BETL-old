...
exec betl.dbo.reset
exec betl.dbo.debug
exec betl.dbo.get_dep_obj_id 188
*/ 
...
--		, @nesting as int
	...
	select 
		@full_obj_name = quotename([db]) + '.'+ quotename([schema]) + '.'+ quotename(obj_name)
		, @obj_schema_name = quotename([schema]) + '.'+ quotename(obj_name)
		, @obj_name = [obj_name]
	    , @obj_type = obj_type
	...
	exec dbo.log @transfer_id, 'VAR', 'object: ? ?(?,?) , server type ?', @obj_type , @obj_name , @obj_id, @from_obj_id, @server_type_id 
	if object_id('tempdb..#dep') is not null
		drop table #dep
...
create table #dep(
	...
	, obj_id int
	primary key (obj_name , schema_name, data_source) 
) 
...
insert into #dep
select distinct lookup_obj.[obj_name], lookup_obj.[schema], "transfer" data_source, lookup_obj.obj_id
from dbo.transfer t
inner join dbo.obj_ext lookup_obj on t.src_obj_id = lookup_obj.obj_id 
where t.target_name = "<full_obj_name>"
'
		set @sql += '
-- step 2. find sql server object dependencies for <full_obj_name>( <obj_id>, <obj_type>) , server type: <server_type_id>

insert into #dep 
SELECT distinct lookup_obj.[obj_name], lookup_obj.[schema], "referenced" data_source, lookup_obj.obj_id
FROM <obj_db>.sys.dm_sql_referenced_entities("<obj_schema_name>", "Object") d
left join dbo.Obj_ext lookup_obj on d.[referenced_schema_name] = lookup_obj.[schema] collate database_default and d.[referenced_entity_name] = lookup_obj.obj_name collate database_default
and lookup_obj.db="<obj_db>" collate database_default
...
insert into #dep 
SELECT distinct s.name schema_name , so.name obj_name, "procedure" data_source, lookup_obj.obj_id
...
left join dbo.Obj_ext lookup_obj on s.name = lookup_obj.[schema] collate database_default  and lookup_obj.obj_name = so.name collate database_default 
	and lookup_obj.db="<obj_db>" collate database_default 
WHERE 
...
--		insert into #dep
--		select 
--		obj.obj_id dep_obj_id, 'ssas to sql mapping'  data_source 
--		--cube_obj.db,
...
--				insert into #dep
--				select 
--				@dep_obj_id dep_obj_id, 'ssas sql parsing'  data_source 
--		end 
--		--print @sql 
...
insert into dbo.Obj_dep(from_obj_id, to_obj_id, data_source) 
select N"<obj_id>", obj_id, data_source
from #dep
except 
select from_obj_id, to_obj_id, data_source
from dbo.Obj_dep
where from_obj_id = "<obj_id>"
'


	delete from @p 		
	insert into @p values('obj_db'					, @obj_db) 
	insert into @p values('from_obj_id'			, @from_obj_id) 
	insert into @p values('obj_id'					, @obj_id) 
	insert into @p values('obj_name'				, @obj_name) 
	insert into @p values('full_obj_name'		, @full_obj_name) 
...
--	exec dbo.log @transfer_id, 'header', '? ? , scope ?, depth ?', @proc_name , @full_obj_name, @scope, @obj_tree_depth
	...
	declare 
		...
		, @debug as bit = 0
	if @debug = 1 
		exec dbo.log @transfer_id, 'Step', 'looking for ?, scope ? ', @full_obj_name , @scope
	
	-- retrieve object meta data. If not found->refresh parent (schema)... 
	Set @obj_id = dbo.obj_id(@full_obj_name, @scope) 
	if @obj_id is null or @obj_id < 0 
	begin 
		if @full_obj_name = 'localhost' -- there should always be a localhost obj.. if not then create. 
		begin
			insert into dbo.Obj(obj_type_id, obj_name)
			values ( 50, 'LOCALHOST')
		...
		--if @debug = 1 
		exec dbo.log @transfer_id, 'Warn', 'object ? not found in scope ? , trying to refresh parent ? ', @full_obj_name, @scope, @parent
		--set @obj_tree_depth +=1
	...
	begin
		if @obj_id is not null and @obj_id >0  
			...
			exec dbo.log @transfer_id, 'ERROR', 'Object ?(?) NOT FOUND', @full_obj_name, @scope, @obj_id
	end	
	-- standard BETL footer code... 
    footer:
...
, obj_dep.data_source data_source
...
, dep.db dep_db
, dep.[schema] dep_schema
, dep.obj_name dep_name
, dep.obj_type dep_type 

from dbo.Obj_dep obj_dep 
inner join dbo.obj_ext obj on obj_dep.from_obj_id = obj.obj_id
inner join dbo.obj_ext dep on obj_dep.to_obj_id = dep.obj_id 
...
select o.[schema], obj_id, scope, full_obj_name,  obj_type, max_transfer_dt
from dbo.Obj_ext o
left join ( 
...
exec dbo.get_dep '[AW_Staging].[dbo]', 
*/ 
...
--	select * from [dbo].[Property_ext]	where obj_id = @obj_id and property like @prop
    footer:
		if @debug=1 
		exec dbo.log @transfer_id =@transfer_id, @log_type ='footerr', @msg ='done ? ? ? for ?', @i1 =@proc_name , @i2 =@prop, @i3 =@value, @i4=@full_obj_name, @simple_mode = 1
	-- END standard BETL footer code... 
end
