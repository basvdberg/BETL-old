...
-- START BETL Release version 3.1.12 , date: 2018-11-22 16:24:35
set nocount on 
use betl 
...
GO
	...
	INSERT [static].[Version] ([major_version], [minor_version], [build], build_dt) VALUES (3,1,12,'2018-11-22 16:24:35')
...
GO
print '-- 2. suffix_first_underscore'
...
GO
print '-- 23. prefix_first_underscore'
...
--	exec dbo.log @transfer_id, 'header', '? ? , scope ?, depth ?', @proc_name , @full_object_name, @scope, @object_tree_depth
	...
	if @debug = 1 
		exec dbo.log @transfer_id, 'Step', 'looking for ?, scope ? ', @full_object_name2 , @scope
	
	-- retrieve object meta data. If not found->refresh schema... 
	Set @obj_id = dbo.obj_id(@full_object_name2, @scope) 
	if @obj_id is null or @obj_id < 0 
	begin 
		...
		begin
			...
			--if @debug = 1 
			exec dbo.log @transfer_id, 'Warn', 'object ? not found in scope ? and no parent ', @full_object_name2, @scope 
			...
			if @obj_id is null or @obj_id < 0 
				exec dbo.log @transfer_id, 'Warn', 'object ? not found', @full_object_name2, @scope 
		...
		--if @debug = 1 
		exec dbo.log @transfer_id, 'Warn', 'object ? not found in scope ? , trying to refresh parent ? ', @full_object_name2, @scope, @parent
		--set @object_tree_depth +=1
...
--			, @attribute_match_str varchar(4000) 
			...
			, @insert_cols_str varchar(4000) 





			, @key_domain_sql as varchar(8000)  = ''
...
--	  @log_level smallint
	...
	, @i6 as sql_variant = null
	, @i7 as sql_variant = null
...
exec dbo.refresh 'bi_ready_dm',0
...
GO
--END BETL Release version 3.1.12 , date: 2018-11-22 16:24:35
